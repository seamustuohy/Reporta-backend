...
class FacebookApiException extends Exception
{
  ...
  public function getType() {
    return
    isset($this->result['error']) && isset($this->result['error']['type']) ? $this->result['error']['type'] : 'Exception';
  }
...
class Facebook
{
  ...
  protected function setCookieFromSession($session=null) {
    ...
    if (headers_sent()) {
      static::errorLog('Could not set cookie. Headers already sent.');
  ...
  protected function validateSessionObject($session) {
    ...
    if (is_array($session) && isset($session['uid']) && isset($session['access_token']) && isset($session['sig'])) {
      ...
      $expected_sig = static::generateSignature(
        $session_without_sig,
        $this->getApiSecret()
        );
      if ($session['sig'] != $expected_sig) {
        static::errorLog('Got invalid session signature in cookie.');
        $session = null;
      }
      // check expiry time
    } else {
  ...
  protected function createSessionFromSignedRequest($data) {
    ...
    $session['sig'] = static::generateSignature(
      $session,
      $this->getApiSecret()
      );
  ...
  protected function parseSignedRequest($signed_request) {
    ...
    // decode the data
    $sig = static::base64UrlDecode($encoded_sig);
    $data = json_decode(static::base64UrlDecode($payload), true);

    if (strtoupper($data['algorithm']) !== 'HMAC-SHA256') {
      static::errorLog('Unknown algorithm. Expected HMAC-SHA256');
      return null;
    }
...
return static::getUrl($name, 'restserver.php');
}
  ...
  protected function getCurrentUrl() {
    ...
    if (!empty($parts['query'])) {
      ...
      foreach(static::$DROP_QUERY_PARAMS as $key) {
        unset($params[$key]);
      }
      if (!empty($params)) {
