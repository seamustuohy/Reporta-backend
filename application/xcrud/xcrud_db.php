...
class Xcrud_db
{
    ...
    public static function get_instance($params = false)
    {
        ...
        if (!isset(static::$_instance[$instance_name]) || null === static::$_instance[$instance_name])
        {
            if (!is_array($params))
            {
            ...
            static::$_instance[$instance_name] = new self($dbuser, $dbpass, $dbname, $dbhost, $dbencoding);
        }
    ...
    public function escape_like($val, $pattern = array('%', '%'))
    {
        ...
        {
            return '\'' . $pattern[0] . ($this->magic_quotes ? (string )$val : $this->connect->real_escape_string((string )$val)) .$pattern[1] . '\'';
        }
    }
    private function error($text = 'Error!')
    {
        exit('<div class="xcrud-error" style="position:relative;line-height:1.25;padding:15px;color:#BA0303;margin:10px;border:1px solid #BA0303;border-radius:4px;font-family:Arial,sans-serif;background:#FFB5B5;box-shadow:inset 0 0 80px #E58989;"> <span style="position:absolute;font-size:10px;bottom:3px;right:5px;">xCRUD</span>' . $text . '</div>');
    }
}
