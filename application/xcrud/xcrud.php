...
// str_replace - windows trick
define('XCRUD_PATH', str_replace('\\', '/', dirname(__file__)));
// configuration
require (XCRUD_PATH . '/xcrud_config.php');
// database class
require (XCRUD_PATH . '/xcrud_db.php');
// sets multibyte encoding globaly
mb_internal_encoding(Xcrud_config::$mbencoding);
// xcrud code not depends on timezone, but this can fix some warnings
...
class Xcrud
{
    ...
    {
        Xcrud_config::$scripts_url = static::check_url(Xcrud_config::$scripts_url, true);
        Xcrud_config::$editor_url = static::check_url(Xcrud_config::$editor_url);
        Xcrud_config::$editor_init_url = static::check_url(Xcrud_config::$editor_init_url);
    ...
    public static function get_instance($name = false)
    {
        ...
        if (!isset(static::$instance[$name]) || null === static::$instance[$name])
        {
            static::$instance[$name] = new self();
            static::$instance[$name]->instance_name = $name;
        }
        static::$instance[$name]->instance_count = count(static::$instance);
    ...
    protected static function init_prepare($method = false)
    {
        ...
        static::session_start($sess_name);
    ...
    public static function session_start($sess_name = false)
    {
        ...
        if (!session_id() && !Xcrud_config::$external_session && !Xcrud_config::$alt_session)
        {
            ...
            {
                static::error('xCRUD can not create session, because the output is already sent into browser. Try to define xCRUD instance before the output start or use session_start() at the beginning of your script');
            }
        }
        if (Xcrud_config::$alt_session)
        {
            if (!headers_sent())
            {
                ...
                {
                    static::$sess_id = base_convert(str_replace(' ', '', microtime()) . rand(), 10, 36);
                }
                else
                {
                    static::$sess_id = $_COOKIE[$sess_name];
                }
                setcookie($sess_name, static::$sess_id, time() + Xcrud_config::$alt_lifetime * 60, '/');
            }
            else
            {
                static::error('xCRUD can not start session, because the output is already sent into browser. Try to define xCRUD instance before the output start or use <strong>Xcrud::session_start();</strong> at the beginning of your script');
            }
        }
    }
        ...
        if ($type)
        {
            ...
            foreach ($fdata as $fitem)
            {
                ...
                switch ($type)
                {
                    ...
                    case 'radio':
                    case 'checkboxes':
                    if (!is_array($attr) || !isset($attr['values']))
                    {
    ...
    public function field_tooltip($fields = '', $tooltip = '', $icon = false)
    {
        if ($fields && $tooltip)
        {
            ...
            foreach ($fdata as $fitem)
            {
                $this->field_tooltip[$fitem['table'] . '.' . $fitem['field']] = array('tooltip' => isset($fitem['value']) ? $fitem['value'] : $tooltip, 'icon' => $icon);
            }
        }
        return $this;
    }
    ...
    public function call_update($postdata, $primary)
    {
        if (!$this->task)
        {
            static::error('Sorry, but you must use <strong>call_update()</strong> only in callbacks');
        }
    ...
    public function get($name = '')
    {
        if (!$this->task)
        {
            static::error('Sorry, but you must use <strong>get()</strong> only in callbacks');
        }
        if ($this->_get('key'))
        {
    ...
    protected function _run_task()
    {
        ...
        {
            return static::error('Task recursion!');
        }
        if (!$this->task)
        {
            $this->task = 'list';
        ...
        switch ($this->task)
        {
            ...
            if (!$this->before)
            {
                return static::error('Restricted task!');
            }
            ...
            if (!$this->is_print)
            {
                return static::error('Restricted');
            }
    ...
    protected function render_custom_csv()
    {
        ...
        // low level result process, saves memory
    ...
    protected function render_output()
    {
        ...
        {
            $contents = '';
            if (!static::$css_loaded && !Xcrud_config::$manual_load)
            {
                $contents .= static::load_css();
            }
            ob_start();
            ...
            ob_end_clean();
            unset($this->data);
            if (!static::$js_loaded && !Xcrud_config::$manual_load)
            {
                $contents .= static::load_js();
            }
            $this->after_render();
            return $contents;
        }
    ...
    public function _csv()
    {
        ...
        foreach ($this->columns as $field => $fitem)
        {
            if (isset($this->field_type[$field]) && ($this->field_type[$field] == 'password' || $this->field_type[$field] == 'hidden'))
            {
                continue;
            }
        ...
        header("Cache-Control: public");
        header("Content-type: application/octet-stream");
        header("Content-Disposition: attachment; filename=\"" . $this->_clean_file_name($this->table_name ? $this->table_name : $this->table) . ".csv\"");
        header("Content-Transfer-Encoding: binary");
        $output = fopen('php://output', 'w');
    ...
    protected function _post($field = '', $default = false, $filter = false)
    {
        if (isset($_POST['xcrud'][$field]))
        {
            ...
            if (($field == 'postdata' || $field == 'unique') && $_POST['xcrud'][$field])
            {
    ...
    protected function _entry($mode = 'edit', $postdata = array())
    {
        ...
        if (((!$this->is_edit($this->result_row) || $this->table_ro) && $mode == 'edit') || (!$this->is_view($this->result_row) && $mode == 'view'))
        {
            return static::error('Forbidden');
        }
    ...
    protected function _insert($postdata, $no_processing = false, $no_processing_fields = array())
    {
        if (!$postdata)
        {
            static::error('$postdata array is empty');
        }
        $set = array();
        $db = Xcrud_db::get_instance($this->connection);
        ...
        {
            if (isset($fields[$key]) && !isset($this->locked_fields[$key]) && !isset($this->custom_fields[$key]) && !isset($this-> disabled[$key]['create']) && !isset($this->readonly[$key]['create']))
            {
                if (isset($this->field_type[$key]))
                {
                ...
                {
                    $set[$fields[$key]['table']]['`' . $fields[$key]['field'] . '`'] = ((isset($this->no_quotes[$key]) && isset($this->pass_var['create'][$key])) ? $db->escape($val, true) : $db->escape($val, false, $this->field_type[$key], $this->field_null[$key], isset($this->bit_field[$key])));
                }
            }
            elseif ($no_processing)
            {
                $set[$no_processing_fields[$key]['table']]['`' . $no_processing_fields[$key]['field'] . '`'] = ((isset($this->no_quotes[$key]) && isset($this->pass_var['create'][$key])) ? $db->escape($val, true) : $db->escape($val, false, $this->field_type[$key], $this->field_null[$key], isset($this->bit_field[$key])));
            }
        }
        
        if (!$set)
        {
            static::error('Nothing to insert');
        }
        if (!$this->primary_ai && !isset($postdata[$this->table . '.' . $this->primary_key]))
        {
            static::error('Can\'t insert a row. No primary value.');
        }
        if (!$this->demo_mode)
        {
    ...
    protected function _update($postdata, $primary)
    {
        ...
        foreach ($postdata as $key => $val)
        {
            if (isset($fields[$key]) && !isset($this->locked_fields[$key]) && !isset($this->custom_fields[$key]) && !isset($this->disabled[$key]['edit']) && !isset($this->readonly[$key]['edit']))
            {
                if (isset($this->field_type[$key]))
                {
                    switch ($this->field_type[$key])
        ...
        if (!$set)
        {
            static::error('Nothing to update');
        }
        if (!$this->join)
        {
            if (!$this->demo_mode)
        ...
        else
        {
            ...
            foreach ($this->join as $alias => $param)
            {
                $joins[] = "INNER JOIN `{$param['join_table']}` AS `{$alias}` ON `{$param['table']}`.`{$param['field']}` = `{$alias}`.`{$param['join_field']}`";
            }
            if (!$this->demo_mode)
            {
    ...
    protected function _remove()
    {
        ...
        else
        {
            ...
            if (in_array('image', $this->field_type) || in_array('file', $this->field_type) || in_array('fk_relation', $this->field_type))
            {
    ...
    protected function _save()
    {
        ...
        // is nested
        if ($this->inner_value !== false)
        {
            $field = reset($this->inner_where);
            // nested table connection field MUST be defined
        ...
        if (!$this->primary_val)
        {
            ...
            if ($this->alert_create)
            {
                foreach ($this->alert_create as $alert)
                {
                    ...
                    if (!$send_to || !preg_match('/^[_a-z0-9-]+(\.[_a-z0-9-]+)*@[a-z0-9-]+(\.[a-z0-9-]+)*(\.[a-z]{2,4})$/', $send_to))
                    {
                        continue;
                    }
            ...
            {
                if (!$this->send_external_create['where_field'] || $postdata[$this->send_external_create['where_field']] == $this->
                    send_external_create['where_val'])
                {
                    foreach ($this->send_external_create['data'] as $key => $value)
                    {
                        $this->send_external_create['data'][$key] = $this->replace_text_variables($value, $postdata + array($this->table . '.' .$this->primary_key => $this->primary_val));
                    }
        ...
        else
        {
            ...
            {
                if (!$this->send_external_edit['where_field'] || $postdata[$this->send_external_edit['where_field']] == $this->send_external_edit['where_val'])
                {
    ...
    protected function call_exception($postdata = array())
    {
        switch ($this->task)
        {
            ...
            switch ($this->_post('type'))
            {
                ...
                break;
                default:
                return static::error('Upload Error');
                break;
            }
            break;
        }
            ...
            if ($val)
            {
                ...
                elseif (isset($this->relation[$field_index]))
                {
                    if (is_array($this->relation[$field_index]['rel_name']))
                    {
                        ...
                        $columns[] = "(SELECT GROUP_CONCAT(DISTINCT (CONCAT_WS('{$this->relation[$field_index]['rel_separator']}'," . implode(',',$tmp_fields) . ")) SEPARATOR ', ') FROM `{$this->relation[$field_index]['rel_tbl']}` WHERE {$where}) AS `rel.{$val['table']}.{$val['field']}`, \r\n `{$val['table']}`.`{$val['field']}` AS `{$val['table']}.{$val['field']}` \r\n";
                    }
                    elseif ($this->relation[$field_index]['multi'])
                    {
                        $columns[] = "(SELECT GROUP_CONCAT(DISTINCT `{$this->relation[$field_index]['rel_name']}` SEPARATOR ', ') FROM `{$this->relation[$field_index]['rel_tbl']}` WHERE FIND_IN_SET(`{$this->relation[$field_index]['rel_tbl']}`.`{$this->relation[$field_index]['rel_field']}`,`{$this->relation[$field_index]['table']}`.`{$this->relation[$field_index]['field']}`) ORDER BY `{$this->relation[$field_index]['rel_name']}` ASC) AS `rel.{$val['table']}.{$val['field']}`, \r\n `{$val['table']}`.`{$val['field']}` AS `{$val['table']}.{$val['field']}` \r\n";
                    }
                    else
                    {
                        $columns[] = "(SELECT `{$this->relation[$field_index]['rel_alias']}`.`{$this->relation[$field_index]['rel_name']}` FROM `{$this->relation[$field_index]['rel_tbl']}` AS `{$this->relation[$field_index]['rel_alias']}` WHERE `{$this->relation[$field_index]['rel_alias']}`.`{$this->relation[$field_index]['rel_field']}` = `{$this->relation[$field_index]['table']}`.`{$this->relation[$field_index]['field']}` LIMIT 1) AS `rel.{$val['table']}.{$val['field']}`, \r\n `{$val['table']}`.`{$val['field']}` AS `{$val['table']}.{$val['field']}` \r\n";
                    }
                }
                ...
                elseif (isset($this->fk_relation[$field_index]))
                {
                    ...
                    $columns[] = '(SELECT GROUP_CONCAT(DISTINCT ' . $rel_name . ' SEPARATOR \', \') FROM `' . $fk['rel_tbl'] . '` INNER JOIN `' . $fk['fk_table'] . '` ON `' . $fk['fk_table'] . '`.`' . $fk['out_fk_field'] . '` = `' . $fk['rel_tbl'] .'`.`' . $fk['rel_field'] . '` WHERE `' . $fk['fk_table'] . '`.`' . $fk['in_fk_field'] . '` = `' . $fk['table'] . '`.`' .$fk['field'] . '` AND ' . $this->_build_rel_where($field_index) . ') AS `' . $fk['alias'] . '` ' . "\r\n";
                }
    ...
    protected function _build_select_details($mode)
    {
        ...
        if ($this->inner_table_instance) // nested table
        {
            ...
            {
                // nested table connection field MUST be extracted from DB, even if not defined
            ...
            foreach ($this->fields as $key => $val)
            {
                if ($val && !isset($this->custom_fields[$key]))
                {
                    ...
                    elseif (isset($this->fk_relation[$key]))
                    {
                        ...
                        $fields[] = '(SELECT GROUP_CONCAT(DISTINCT `' . $fk['rel_tbl'] . '`.`' . $fk['rel_field'] . '` SEPARATOR \',\') FROM `' . $fk['rel_tbl'] . '` INNER JOIN `' . $fk['fk_table'] . '` ON `' . $fk['fk_table'] . '`.`' . $fk['out_fk_field'] . '` = `' . $fk['rel_tbl'] .'`.`' . $fk['rel_field'] . '` WHERE `' . $fk['fk_table'] . '`.`' . $fk['in_fk_field'] . '` = `' . $fk['table'] . '`.`' .$fk['field'] . '` AND ' . $this->_build_rel_where($key) . ') AS `' . $fk['alias'] . '` ' . "\r\n";
                    }
                    elseif (isset($this->point_field[$key]))
                    {
    ...
    protected function _build_table_join()
    {
        ...
        if (count($this->join))
        {
            ...
            {
                $join_arr[] = "INNER JOIN `{$params['join_table']}` AS `{$alias}` ON `{$params['table']}`.`{$params['field']}` = `{$alias}`.`{$params['join_field']}`";
            }
    ...
    protected function _build_where()
    {
        ...
        // search condition
        if ($this->search && ($this->task == 'list' || $this->task == 'print' || $this->task == 'csv' || $this->after == 'list'))
        {
            if ($where_arr)
            {
                $where_arr[] = 'AND';
            ...
            if ($this->column && isset($this->search_columns[$this->column]))
            {
                ...
                elseif (isset($this->point_field[$this->column]))
                {
                    ...
                    $fitem = reset($fdata);
                    $where_arr[] = 'CONCAT(X(`' . $fitem['table'] . '`.`' . $fitem['field'] . '`),\',\',Y(`' . $fitem['table'] . '`.`' . $fitem['field'] .'`))LIKE ' . $db->escape_like($this->phrase, $this->search_pattern);
                }
                else
                {
                    $fdata = $this->_parse_field_names($this->column, 'build_where', false, false);
...
/** relation values will be searched by displayed name (not by id) */
    protected function _build_relation_subwhere($key) // multicolumn name
    {
        ...
        if ($key)
        {
            ...
            if (is_array($rel['rel_name']))
            {
                ...
                $select = "(SELECT GROUP_CONCAT(DISTINCT (CONCAT_WS('{$rel['rel_separator']}'," . implode(',', $tmp_fields) .")) SEPARATOR ', ') FROM `{$rel['rel_tbl']}` WHERE {$where})\r\n";
            }
            // multiselect relation
            elseif ($rel['multi'])
            {
                $select = "(SELECT GROUP_CONCAT(DISTINCT `{$rel['rel_name']}` SEPARATOR ', ') FROM `{$rel['rel_tbl']}` WHERE FIND_IN_SET(`{$rel['rel_tbl']}`.`{$rel['rel_field']}`,`{$rel['table']}`.`{$rel['field']}`) ORDER BY `{$rel['rel_name']}` ASC)\r\n";
            }
            else
            {
                $select = "(SELECT `{$rel['rel_alias']}`.`{$rel['rel_name']}` FROM `{$rel['rel_tbl']}` AS `{$rel['rel_alias']}` WHERE `{$rel['rel_alias']}`.`{$rel['rel_field']}` = `{$rel['table']}`.`{$rel['field']}` LIMIT 1) \r\n";
            }
    ...
    // multicolumn name
    protected function _build_fk_relation_subwhere($key)
    {
        $db = Xcrud_db::get_instance($this->connection);
        $fk = $this->fk_relation[$key];
        ...
        $select = '(SELECT GROUP_CONCAT(DISTINCT ' . $rel_name . ' SEPARATOR \', \') FROM `' . $fk['rel_tbl'] . '`INNER JOIN `' . $fk['fk_table'] . '` ON `' . $fk['fk_table'] . '`.`' . $fk['out_fk_field'] . '` = `' . $fk['rel_tbl'] .'`.`' . $fk['rel_field'] . '` WHERE `' . $fk['fk_table'] . '`.`' . $fk['in_fk_field'] . '` = `' . $fk['table'] . '`.`' .$fk['field'] . '` AND ' . $this->_build_rel_where($key) . ')' . "\r\n";
        return $select . ' LIKE ' . $db->escape_like($this->phrase, $this->search_pattern);
    }
    ...
    protected function _build_order_by()
    {
        if (count($this->order_by))
        {
            ...
            foreach ($this->order_by as $field => $direction)
            {
                ...
                elseif (isset($this->subselect[$field]) || isset($this->columns[$field]) || isset($this->no_select[$field]) || !strpos($field,'.'))
                {
            ...
            foreach ($this->table_info as $table => $types)
            {
                foreach ($types as $row)
                {
                    ...
                    if ($row['Key'] == 'PRI' && $row['Extra'] == 'auto_increment')
                    {
                        ...
                        if (((!$this->show_primary_ai_column && $mode == 'list') || (!$this->show_primary_ai_field && $mode != 'list')) && !isset($fields_object[$field_index]))
                        {
            ...
            if (!$this->primary_key)
            {
                ...
                else
                {
                    switch ($this->task)
                    {
                        ...
                        break;
                        default:
                        static::error('<strong>Table "' . $this->table . '" has no any primary or unique key!</strong><br />This error was made to prevent loss of your data. You must create primary key (the best - primary autoincrement) for this table. See documentation for more info.');
                        break;
                    }
                }
            }
    ...
    protected function _define_field_type($row, $field_index)
    {
        ...
        if ($max_l && (!isset($this->field_attr[$field_index]['maxlength']) || !$this->field_attr[$field_index]['maxlength']))
        {
    ...
    protected function _render_details($mode)
    {
        ...
        if ($this->inner_table_instance && ($mode == 'view' || $mode == 'edit')) // restoring nested objects
        {
            foreach ($this->inner_table_instance as $inst_name => $field)
            {
                if (isset($this->result_row[$field]))
                {
                    $instance = static::get_instance($inst_name);
        ...
        // session auto-clearing, must start on first instance
            if ($this->instance_count == 1 && !$this->ajax_request)
            {
                if (isset($_SESSION['xcrud_session']) && $_SESSION['xcrud_session'])
                {
                    ...
                    // autocleaner
                        if ($time > $old_time + Xcrud_config::$autoclean_timeout)
                        {
            ...
            if (Xcrud_config::$alt_session)
            {
                ...
                if (class_exists('Memcache'))
                {
                    ...
                    $res = $mc->set(static::$sess_id, $data, false, Xcrud_config::$alt_lifetime * 60);
                }
                elseif (class_exists('Memcached'))
                {
                    $mc = new Memcached();
                    $mc->connect(Xcrud_config::$mc_host, Xcrud_config::$mc_port);
                    $res = $mc->set(static::$sess_id, $data, Xcrud_config::$alt_lifetime * 60);
                }
                else
                {
                    static::error('Can\'t use alternative session. Memcache(d) is not available');
                }
                unset($_SESSION['xcrud_session']);
                if (!$res)
                {
                    static::error('Can\'t use alternative session. Memcache(d) has invalid parameters or broken. Storing failed');
                }
            }
...
public function import_vars($key = false)
{
    if (Xcrud_config::$alt_session)
    {
        ...
        if (!$data)
        {
            static::error('Can\'t use alternative session. Data is not exist');
        }
        $_SESSION['xcrud_session'] = $this->decrypt($data[0], $data[1]);
        unset($data);
        if (!$_SESSION['xcrud_session'])
        {
            static::error('Can\'t use alternative session. Data is invalid');
        }
    }
...
protected function create_view_price($name, $value = '', $tag = array())
{
    ...
    $out .= number_format($value ? $value : 0, $this->field_attr[$name]['decimals'], $this->field_attr[$name]['point'], $this->html_safe($this->field_attr[$name]['separator']));
...
protected function create_relation($name, $value = '', $tag = array(), $dependval = false)
{
    ...
    else
    {
        ...
        $db->query('SELECT `' . $this->relation[$name]['rel_field'] . '` AS `field`,' . $name_select . $this->get_relation_tree_fields($this->relation[$name]) . ' FROM `' . $this->relation[$name]['rel_tbl'] . '` ' . $where .' GROUP BY `field` ORDER BY ' . $this->get_relation_ordering($this->relation[$name]));
...
protected function create_image($name, $value = '', $tag = array(), $is_upload = false)
{
    if (isset($this->subselect[$name]))
    {
        ...
        if ($value)
        {
            ...
            $attr = array('src' => isset($this->upload_config[$name]['url']) ? $this->real_file_link($value, $this->upload_config[$name], true) : $this->file_link($name, $this->primary_val, (isset($this->upload_config[$name]['detail_thumb']) ? $this->upload_config[$name]['detail_thumb'] : false), false, $value), 'alt' => '');
    ...
    protected function create_point($name, $value = '', $tag = array())
    {
        ...
        if (isset($this->readonly[$name]) || isset($this->disabled[$name]))
        {
    ...
    protected function _upload_file()
    {
        ...
        {
            return static::error('File is not uploaded');
        }
    }
    ...
    protected function _remove_upload()
    {
        ...
        switch ($type)
        {
            ...
            break;
            default:
            return static::error('Remove Error');
            break;
        }
    }
    ...
    protected function _image_resize($src_file, $dest_file, $new_size_w = false, $new_size_h = false, $dest_qual = 92, $watermark = false, $watermark_position = array(50, 50))
    {
        ...
        if ($watermark)
        {
            ...
            switch ($water_type)
            {
                ...
                break;
                default:
                static::error('NO WATERMARK FILE');
                return false;
            }
        ...
        switch ($type)
        {
            ...
            break;
            default:
            static::error('File Type Not Supported!');
            return false;
        }
    ...
    protected function _image_crop($src_file, $dest_file, $new_size_w, $new_size_h, $dest_qual = 92, $watermark = false, $watermark_position = array(50, 50))
    {
        ...
        if (!$srcHandle)
        {
            static::error('Could not execute imagecreatefrom() function! ');
            return false;
        }
        ...
        {
            static::error('Could not execute imagecopyresampled() function!');
            return false;
        }
        imagedestroy($srcHandle);
        if ($watermark)
    ...
    {
        $offsets = array();
        $pos_x = ($water_pos[0] < 0 || $water_pos[0] > 100) ? 0 : $water_pos[0];
        $pos_y = ($water_pos[1] < 0 || $water_pos[1] > 100) ? 0 : $water_pos[1];
...
public static function check_url($url, $scr_url = false)
{
    ...
    $scheme = (!isset($_SERVER['HTTPS']) || !$_SERVER['HTTPS'] || strtolower($_SERVER['HTTPS']) == 'off' || strtolower($_SERVER['HTTPS']) =='no') ? 'http://' : 'https://';
    ...
    {
        if (mb_substr($url, 0, 1) == '/' || mb_substr($url, 0, 2) == './')
        {
    ...
    if (is_array($this->table_info) && count($this->table_info) && !$this->table_ro)
    {
        ...
        foreach ($this->table_info as $table => $types)
        {
            foreach ($types as $row)
            {
                ...
                {
                    static::error('Duplication impossible. The table has a unique field.');
                }
                else
                {
        ...
        if (!$this->primary_ai)
        {
            static::error('Duplication impossible. Table does not have a primary autoincrement field.');
        }
...
protected function _get_table($method)
{
    ...
    {
        static::error('You must define your table before using the <strong>' . $method . '</strong> method.');
    }
    else
    {
    ...
    protected function _get_theme_config()
    {
        ...
        {
            static::error('xcrud.ini does not exist in your theme folder');
        }
    }
    ...
    public function _parse_field_names($fields = '', $location = '', $table = false, $insert_prefix = true)
    {
        ...
        {
            static::error('You must set field name(s) for the <strong>' . $location . '</strong> method.');
        }
        return $field_names;
    }
    ...
    public static function load_css()
    {
        ...
        if (!static::$js_loaded && !static::$instance)
        {
            Xcrud_config::$scripts_url = static::check_url(Xcrud_config::$scripts_url, true);
            Xcrud_config::$editor_url = static::check_url(Xcrud_config::$editor_url);
            Xcrud_config::$editor_init_url = static::check_url(Xcrud_config::$editor_init_url);
        ...
        if (static::$css_loaded)
        {
            static::error('Xcrud\'s styles already rendered! Please, set <strong>$manual_load = true</strong> in your configuration file');
        }

        static::$css_loaded = true;
        if (Xcrud_config::$load_bootstrap)
        {
            $out .= '<link href="' . Xcrud_config::$scripts_url . '/' . Xcrud_config::$plugins_uri .'/bootstrap/css/bootstrap.min.css" rel="stylesheet" type="text/css" />';
        }
        if (Xcrud_config::$load_jquery_ui)
        {
            $out .= '<link href="' . Xcrud_config::$scripts_url . '/' . Xcrud_config::$plugins_uri .'/jquery-ui/jquery-ui.min.css" rel="stylesheet" type="text/css" />';
        }
        if (Xcrud_config::$load_jcrop)
        {
            $out .= '<link href="' . Xcrud_config::$scripts_url . '/' . Xcrud_config::$plugins_uri .'/jcrop/jquery.Jcrop.min.css" rel="stylesheet" type="text/css" />';
        }
        $out .= '<link href="' . Xcrud_config::$scripts_url . '/' . Xcrud_config::$plugins_uri .'/timepicker/jquery-ui-timepicker-addon.css" rel="stylesheet" type="text/css" /><link href="' . Xcrud_config::$scripts_url .'/' . Xcrud_config::$themes_uri . '/' . Xcrud_config::$theme . '/xcrud.css" rel="stylesheet" type="text/css" />';
        return $out;
    }
    public static function load_js()
    {
        ...
        if (static::$instance)
        {
            $instance = reset(static::$instance);
        ...
        else
        {
            ...
            $lang_arr = static::_get_language_static();
        }

        if (!static::$css_loaded && !static::$instance)
        {
            Xcrud_config::$scripts_url = static::check_url(Xcrud_config::$scripts_url, true);
            Xcrud_config::$editor_url = static::check_url(Xcrud_config::$editor_url);
            Xcrud_config::$editor_init_url = static::check_url(Xcrud_config::$editor_init_url);
        ...
        if (static::$js_loaded)
        {
            static::error('Xcrud\'s scripts already rendered! Please, set <strong>$manual_load = true</strong> in your configuration file');
        }
        static::$js_loaded = true;
...
protected function check_file($path, $func_name)
{
    ...
    if (!is_file($path))
    {
        static::error('Wrong path or file is not exist! The <strong>' . $func_name . '</strong> method fails.<br /><small>' . $path .'</small>');
    }
    return $path;
}
protected function check_folder($path, $func_name)
{
    ...
    if (!is_dir($path))
    {
        if (!@mkdir($path))
        {
            static::error('Wrong path or folder is not exist! The <strong>' . $func_name . '</strong> method fails.<br /><small>' . $path .'</small>');
        }
    }
    return $path;
}
...
protected function additional_columns($fields = '')
{
    ...
    protected function render_fields_list($mode, $container = 'table', $row = 'tr', $label = 'td', $field = 'td', $tabs_block = 'div', $tabs_head = 'ul', $tabs_row = 'li', $tabs_link = 'a', $tabs_content = 'div', $tabs_pane = 'div')
    {
        ...
        if (isset($this->field_tabs[$mode]) || $this->default_tab !== false)
        {
            ...
            if ($this->default_tab !== false && $raw_out)
            {
                ...
                $tabs_header .= $this->open_tag($tabs_row, $this->theme_config('tabs_header_cell') . ($k == 0 ? ' ' . $this->theme_config('tabs_first_element') : '')) . $this->open_tag($tabs_link, $this->theme_config('tabs_header_link') . ($k ==0 ? ' ' . $this->theme_config('tabs_first_element') : ''), array('href' => '#' . $tabid)) . $this->default_tab . $this->close_tag($tabs_link) . $this->close_tag($tabs_row);
                $tabs_body .= $this->open_tag($tabs_pane, $this->theme_config('tabs_content_pane') . ($k == 0 ? ' ' . $this->theme_config('tabs_first_element') : ''), array('id' => $tabid)) . $this->open_tag($container, $this->theme_config('details_container')) .implode('', $raw_out) . $this->close_tag($container) . $this->close_tag($tabs_pane);
                ++$k;
                $raw_out = array();
            }
            ...
            if (isset($this->field_tabs[$mode]) && $tabs_out)
            {
                foreach ($this->field_tabs[$mode] as $key => $tabname)
                {
                    ...
                    {
                        $tabid = 'tabid_' . base_convert(rand(), 10, 36);
                        $tabs_header .= $this->open_tag($tabs_row, $this->theme_config('tabs_header_cell') . ($k == 0 ? ' ' . $this->theme_config('tabs_first_element') : '')) . $this->open_tag($tabs_link, $this->theme_config('tabs_header_link') . ($k ==0 ? ' ' . $this->theme_config('tabs_first_element') : ''), array('href' => '#' . $tabid)) . $tabname . $this->close_tag($tabs_link) .$this->close_tag($tabs_row);
                        $tabs_body .= $this->open_tag($tabs_pane, $this->theme_config('tabs_content_pane') . ($k == 0 ? ' ' . $this->theme_config('tabs_first_element') : ''), array('id' => $tabid)) . $this->open_tag($container, $this->theme_config('details_container')) .implode('', $tabs_out[$tabname]) . $this->close_tag($container) . $this->close_tag($tabs_pane);
                        ++$k;
                    }
                }
            }
    ...
    protected function render_search()
    {
        ...
        if ($this->is_search)
        {
            ...
            {
                $out .= $this->open_tag(array('tag' => 'input', 'type' => 'hidden'), 'xcrud-columns-select ' . $this->theme_config('search_fieldlist'),$attr);
            }
            else
            {
    ...
    protected function search_fieldlist($field, $phrase, $fieldlist)
    {
        ...
        switch ($attr['data-type'])
        {
            ...
            case 'float':
            default:
            if (!isset($fieldlist['default']) || $field == $this->column)
            {
                if (!$this->column)
                {
                    $class = 'xcrud-search-active';
                ...
                $attr['type'] = 'text';
                $attr['value'] = (!is_array($phrase) && $field == $this->column || !$this->column) ? $phrase : '';
                $fieldlist['default'] = $this->single_tag('input', $class . ' ' . $this->theme_config('search_phrase'), $attr);
            }
            break;
            case 'bool':
            if (!isset($fieldlist['bool']) || $field == $this->column)
            {
            ...
            case 'time':
            case 'timestamp':
            if (!isset($fieldlist['date']) || $field == $this->column)
            {
    ...
    protected function render_grid_head($row = array('tag' => 'tr'), $item = array('tag' => 'th'), $arrows = array('asc' =>'&uarr; ', 'desc' => '&darr; '))
    {
        $out = '';
    ...
    protected function render_grid_body($row_tag = array('tag' => 'tr'), $item = array('tag' => 'td'))
    {
        ...
        else
        {
            ...
            $out .= $this->open_tag($row_tag, 'xcrud-row') . $this->open_tag($item, '', array('colspan' => $j)) . $this->lang('table_empty') .$this->close_tag($item) . $this->close_tag($row_tag);
        }
        return $out;
    }
    ...
    protected function create_file_folders($path)
    {
        ...
        if (is_dir(implode('/', $path_array)))
        {
            if (!mkdir($path))
            {
                static::error('cannot create directory ' . $path);
            }
        }
        else
        {
            static::error('File path is incorrect!');
        }
    }
    ...
    {
        if (isset($this->upload_config[$field]['width']) || isset($this->upload_config[$field]['height']) || (isset($this->upload_config[$field]['manual_crop']) && $this->upload_config[$field]['manual_crop'] == true) )
        {
            return true;
        }
        else
    ...
    protected function unix2datetime($time, $utc = false)
    {
        if ($time)
        {
            return $utc ? gmdate($this->date_format['php_d'] . ' ' . $this->date_format['php_t'], $time) : date(Xcrud_config::$php_date_format .' ' . $this->date_format['php_t'], $time);
        }
        else
        {
            return '';
    ...
    protected function render_table_name($mode = 'list', $tag = 'h2', $to_show = false)
    {
        ...
        if ($this->is_title)
        {
            ...
            {
                case 'create':
                $out .= $this->is_rtl ? '<small>' . $this->lang('add') . ' - </small>' . $this->table_name : $this->table_name .'<small> - ' . $this->lang('add') . '</small>';
                break;
                case 'edit':
                $out .= $this->is_rtl ? '<small>' . $this->lang('edit') . ' - </small>' . $this->table_name : $this->table_name .'<small> - ' . $this->lang('edit') . '</small>';
                break;
                case 'view':
                $out .= $this->is_rtl ? '<small>' . $this->lang('view') . ' - </small>' . $this->table_name : $this->table_name .'<small> - ' . $this->lang('view') . '</small>';
                break;
                default:
                $out .= $this->is_rtl ? '<small>' . $this->get_table_tooltip() . '</small>' . $this->table_name : $this->table_name .'<small> ' . $this->get_table_tooltip() . '</small>';
                break;
            }
    ...
    public function encrypt($obj)
    {
        ...
        {
            static::error('Please, set <strong>$alt_encription_key</strong> parameter in configuration file');
        }
        $text = json_encode($obj);

        if (!is_callable('mcrypt_module_open'))
        {
            static::error('<strong>mcrypt_module</strong> not found');
        }
        ...
        {
            static::error('MCRYPT - Supported algorytm not found');
        }
    ...
    public function load_core_class($name)
    {
        ...
        if (is_file($path))
        {
            ...
            {
                static::$classes[$name] = new $class;
                return static::$classes[$name];
            }
            else
            {
                static::error('Class "' . $class . '" not exist!');
            }
        }
        else
        {
            static::error('File "' . $name . '.php" not exist!');
        }
    }
